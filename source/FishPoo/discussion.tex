...
Rather than comparing candidate cases to a model, a comparative approach calls for a metric that scales with dissimilarity among cases. Then 
...
\begin{equation}
    S( p(x), q(x) ) = \int_{-\infty}^\infty p(x) \, \log\frac{p(x)}{q(x)} \, {\rm d}x
\end{equation}

\begin{equation}
    D( p(x), q(x) ) = \sqrt{ \frac{1}{2} S( p(x), q(x) ) + \frac{1}{2} S( q(x), p(x) ) }
\end{equation}
